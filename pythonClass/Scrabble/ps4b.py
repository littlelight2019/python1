...
def playGame(wordList):
    ...
    """
    size = 7
    hand = ps4a.dealHand(size)
    while True:
        choice = input("Enter a choice: n for new hand, r to use current hand, e to exit: ")
        while choice not in 'ern':
            choice = input("Enter a choice: n for new hand, r to use current hand, e to exit: ")
        if choice == 'e':
            break
        if choice == 'n':
        ...
        print("Current Hand:", end=' ')
        ps4a.displayHand(hand)
        player = input("Enter u for user, c for computer: ")
        while player not in 'cu':
            player = input("Enter u for user, c for computer: ")        
        if player == 'c':
            compPlayHand(hand, wordList, size)
        else:
            ps4a.playHand(hand, wordList, size)
